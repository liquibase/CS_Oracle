...
--rollback drop table TEMP_table2_x
...
CREATE TABLE "SWPRX_BLAH" 
  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "SWPRX_BLAH_TRG" BEFORE
    INSERT OR UPDATE ON SWPRX_BLAH
    FOR EACH ROW
BEGIN
    IF INSERTING THEN
        IF :NEW.AUDIT_LOG_ID IS NULL THEN
            :NEW.AUDIT_LOG_ID := SWPRX_AUDIT_LOG_SEQ.NEXTVAL;
        END IF;
...
/
ALTER TRIGGER "SWPRX_BLAH_TRG" ENABLE;
...
CREATE TABLE "SWPRC_BLAH" 
  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "SWPRC_BLAH_TRG" BEFORE
    INSERT OR UPDATE ON SWPRC_BLAH
    FOR EACH ROW
BEGIN
    IF INSERTING THEN
        IF :NEW.AUDIT_LOG_ID IS NULL THEN
            :NEW.AUDIT_LOG_ID := SWPRC_AUDIT_LOG_SEQ.NEXTVAL;
        END IF;
...
/
ALTER TRIGGER "SWPRC_BLAH_TRG" ENABLE;


--changeset customer:test3_bad
CREATE TABLE "HELLO_SWPRC_BLAH" 
   (	"AUDIT_LOG_ID" NUMBER, 
	"CHANGED_TABLE_NAME" VARCHAR2(50 BYTE), 
	"INSERT_USER" VARCHAR2(100 BYTE), 
	"INSERT_DATE" DATE ,
...
CREATE TABLE "SWPRC_BLAH" 
  ...
  CREATE OR REPLACE EDITIONABLE TRIGGER "BAD_SWPRC_BLAH_TRG" BEFORE
    INSERT OR UPDATE ON SWPRC_BLAH
    FOR EACH ROW
BEGIN
    IF INSERTING THEN
...
/
ALTER TRIGGER "BAD_SWPRC_BLAH_TRG" ENABLE;
